...
const ADMIN_PASSWORD = "admin2026"; // ← đổi mật khẩu ở đây
...
const ALERT_LEVELS = { critical:{label:"Nghiêm trọng",color:"#dc2626",bg:"#fef2f2",icon:"🔴"}, warning:{label:"Cảnh báo",color:"#d97706",bg:"#fffbeb",icon:"🟡"}, info:{label:"Thông tin",color:"#2563eb",bg:"#eff6ff",icon:"🔵"} };
...
const SIDEBAR_MENU = [
    ...
    { key:"sos",        icon:"🚨", label:"Điều phối SOS" },
    { key:"map",        icon:"🗺️",  label:"Bản đồ Live" },
    { key:"volunteers", icon:"👥", label:"Tình nguyện viên" },
    { key:"supplies",   icon:"📦", label:"Vật tư cứu trợ" },
    { key:"stations",   icon:"🏠", label:"Điểm cứu trợ" },
    { key:"donations",  icon:"🎁", label:"Quyên góp" },
...
function Modal({ open, title, onClose, children }) {
    ...
    return (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}
             onClick={onClose}>
            <div style={{ background:"#fff", borderRadius:20, padding:"28px 28px", width:"100%", maxWidth:520, maxHeight:"85vh", overflowY:"auto", boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}
        ...
        <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0f172a,#1e293b)", display:"flex", alignItems:"center", justifyContent:"center" }}>
            <div style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", backdropFilter:"blur(20px)", borderRadius:24, padding:"48px 40px", width:380, boxShadow:"0 24px 80px rgba(0,0,0,0.4)" }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                    ...
                    {err && <p style={{ fontSize:12, color:"#f87171", margin:0 }}>❌ Mật khẩu không đúng</p>}
                    <button onClick={submit} style={{ background:"linear-gradient(135deg,#16a34a,#15803d)", color:"white", border:"none", borderRadius:12, padding:14, fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit", boxShadow:"0 4px 20px rgba(22,163,74,0.4)" }}>
                        Đăng nhập →
                    </button>
                </div>
            </div>
    ...
    return (
        <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14 }}>
                <StatCard icon="🆘" value={urgent}  label="Khẩn cấp"    color="#dc2626" sub="cần hỗ trợ ngay" />
                <StatCard icon="🚑" value={helping} label="Đang hỗ trợ" color="#2563eb" sub="đang xử lý" />
                <StatCard icon="✅" value={done}    label="Hoàn thành"  color="#16a34a" sub="hôm nay" />
                <StatCard icon="🔔" value={critAlerts} label="Cảnh báo đỏ" color="#dc2626" sub="chưa xử lý" />
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14 }}>
                <StatCard icon="📋" value={sos.length}        label="Tổng SOS"          color="#6366f1" sub="tất cả thời gian" />
                <StatCard icon="👥" value={volunteers.length} label="Tình nguyện viên"  color="#0891b2" sub="đã đăng ký" />
                <StatCard icon="🎁" value={donations.length}  label="Lượt quyên góp"    color="#d97706" sub="đã nhận" />
            </div>

            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                {/* Province chart */}
                <div style={{ background:"#fff", borderRadius:16, padding:20, border:"1px solid #f1f5f9" }}>
                    <h3 style={{ fontSize:14, fontWeight:700, color:"#111827", margin:"0 0 16px" }}>📍 Khu vực nổi bật</h3>
                ...
                <div style={{ background:"#fff", borderRadius:16, padding:20, border:"1px solid #f1f5f9" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
                        ...
                        <button onClick={()=>setPage("sos")} style={{ fontSize:12, color:"#16a34a", background:"none", border:"none", cursor:"pointer", fontFamily:"inherit", fontWeight:600 }}>Xem tất cả →</button>
                    </div>
                    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                        {sos.slice(0,5).map(r=>(
                            <div key={r.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"8px 0", borderBottom:"1px solid #f9fafb" }}>
                                <div>
                                    ...
                                    <div style={{ fontSize:11, color:"#9ca3af" }}>{r.province} · {timeAgo(r.createdAt)}</div>
                                </div>
            ...
            <div style={{ background:"linear-gradient(135deg,#0f172a,#1e293b)", borderRadius:16, padding:20, display:"flex", gap:12, alignItems:"center" }}>
                ...
                {[{page:"sos",label:"Điều phối SOS",emoji:"🚨"},{page:"map",label:"Bản đồ Live",emoji:"🗺️"},{page:"alerts",label:"Cảnh báo",emoji:"🔔"},{page:"supplies",label:"Vật tư",emoji:"📦"}].map(b=>(
        ...
        <div>
            <div style={{ display:"flex", gap:8, marginBottom:16, flexWrap:"wrap", alignItems:"center" }}>
                ...
                <input placeholder="🔍 Tìm kiếm..." value={search} onChange={e=>setSearch(e.target.value)}
                       style={{ marginLeft:"auto", border:"1px solid #e5e7eb", borderRadius:10, padding:"7px 12px", fontSize:12, outline:"none", fontFamily:"inherit", width:220, color:"#111827" }} />
            </div>
                ...
                {filtered.map(item=>(
                    <div key={item.id} style={{ background:"#fff", border:"1px solid #f1f5f9", borderRadius:14, padding:"14px 18px", display:"grid", gridTemplateColumns:"1.2fr 1fr 1fr 1fr auto", gap:12, alignItems:"center", borderLeft:`3px solid ${S[item.status]?.color||"#e5e7eb"}` }}>
                        <div>
                            ...
                            <div style={{ fontSize:11, color:"#6b7280", marginTop:2 }}>📞 {item.phone}</div>
                            <div style={{ fontSize:10, color:"#9ca3af", marginTop:1 }}>🕒 {timeAgo(item.createdAt)}</div>
                        </div>
                        <div>
                            <div style={{ fontSize:12, fontWeight:600, color:"#374151" }}>📍 {item.province}</div>
                        ...
                        <div style={{ fontSize:12, color:"#374151" }}>🏥 {item.supportType||"—"}</div>
                        <div><Badge status={item.status} /></div>
                        <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
                            <button onClick={()=>handleStatus(item)} disabled={busy===item.id} style={{ background:"#16a34a", color:"white", border:"none", borderRadius:8, padding:"6px 10px", fontSize:11, fontWeight:700, cursor:"pointer", fontFamily:"inherit", opacity:busy===item.id?0.6:1, whiteSpace:"nowrap" }}>
                                {busy===item.id?"...": `→ ${S[NEXT[item.status]]?.label||"Hỗ trợ"}`}
                            </button>
                            <button onClick={()=>window.confirm("Xoá?")&&deleteDocument("sos_requests",item.id)} style={{ background:"#fef2f2", color:"#dc2626", border:"1px solid #fca5a5", borderRadius:8, padding:"5px 10px", fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>
                                🗑 Xoá
                            </button>
                        </div>
                    </div>
    ...
    return (
        <div>
            <div style={{ display:"flex", gap:10, marginBottom:16, alignItems:"center" }}>
                <input placeholder="🔍 Tìm kiếm..." value={search} onChange={e=>setSearch(e.target.value)}
                       style={{ border:"1px solid #e5e7eb", borderRadius:10, padding:"7px 12px", fontSize:12, outline:"none", fontFamily:"inherit", width:220, color:"#111827" }} />
                ...
                {filtered.map(v=>(
                    <div key={v.id} style={{ background:"#fff", border:"1px solid #f1f5f9", borderRadius:14, padding:16, borderLeft:"3px solid #16a34a" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
                            <div style={{ width:40, height:40, borderRadius:12, background:"linear-gradient(135deg,#dcfce7,#bbf7d0)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18 }}>🧑‍🚒</div>
                            <div>
                        ...
                        <div style={{ display:"flex", gap:6, marginBottom:8 }}>
                            ...
                            <Chip color="#2563eb">⏰ {v.availability==="fulltime"?"Full":(v.availability==="weekend"?"CuốiTuần":(v.availability==="evening"?"BuổiTối":"Linh hoạt"))}</Chip>
                        </div>
...
function SuppliesPage({ supplies }) {
    ...
    const getStatus = (item) => {
        const pct = item.minQuantity>0 ? item.quantity/item.minQuantity : 1;
        if (pct <= 0) return { label:"Hết", color:"#dc2626", bg:"#fef2f2" };
        if (pct < 1)  return { label:"Sắp hết", color:"#d97706", bg:"#fffbeb" };
    ...
    return (
        <div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(6,1fr)", gap:10, marginBottom:16 }}>
                {Object.entries(SUPPLY_CAT).map(([k,v])=>(
                    <div key={k} style={{ background:"#fff", border:"1px solid #f1f5f9", borderRadius:12, padding:"12px 14px", textAlign:"center" }}>
                        <div style={{ fontSize:20, marginBottom:4 }}>{k==="food"?"🍲":k==="water"?"💧":k==="medicine"?"💊":k==="clothes"?"👕":k==="equipment"?"🔧":"📦"}</div>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
                ...
                <button onClick={()=>setModal(true)} style={{ background:"#16a34a", color:"white", border:"none", borderRadius:10, padding:"9px 18px", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
                    + Thêm vật tư
                </button>
            </div>
            ...
            <div style={{ background:"#fff", borderRadius:16, border:"1px solid #f1f5f9", overflow:"hidden" }}>
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                    ...
                    <tbody>
                    {supplies.length===0 && (
                        <tr><td colSpan={7} style={{ textAlign:"center", padding:"50px 0", color:"#9ca3af" }}>Chưa có vật tư nào. Bấm "+ Thêm vật tư" để bắt đầu.</td></tr>
                    )}
                    {supplies.map(item=>{
                        ...
                        return (
                            <tr key={item.id} style={{ borderBottom:"1px solid #f9fafb" }}>
                                ...
                                <td style={{ padding:"12px 14px", color:"#6b7280" }}>📍 {item.location}</td>
                                <td style={{ padding:"12px 14px" }}>
                                ...
                                <td style={{ padding:"12px 14px" }}>
                                    <button onClick={()=>window.confirm("Xoá?")&&deleteDocument("supplies",item.id)} style={{ background:"none", border:"none", cursor:"pointer", color:"#dc2626", fontSize:14 }}>🗑</button>
                                </td>
                            </tr>
                        );
                    })}
            ...
            <Modal open={modal} title="➕ Thêm vật tư cứu trợ" onClose={()=>setModal(false)}>
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                    ...
                    <button onClick={handleSave} disabled={saving||!form.name||!form.quantity} style={{ background:"#16a34a", color:"white", border:"none", borderRadius:12, padding:13, fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit", opacity:saving?0.7:1, marginTop:8 }}>
    ...
    return (
        <div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14, marginBottom:16 }}>
                <StatCard icon="🎁" value={donations.length} label="Tổng lượt" color="#d97706" sub="quyên góp" />
                <StatCard icon="💰" value={`${fmtNum(total)}đ`} label="Tổng tiền" color="#16a34a" sub="ước tính" />
                <StatCard icon="📦" value={donations.filter(d=>d.donationType!=="money").length} label="Hiện vật" color="#2563eb" sub="không phải tiền" />
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                ...
                {donations.map(d=>(
                    <div key={d.id} style={{ background:"#fff", border:"1px solid #f1f5f9", borderRadius:14, padding:"14px 18px", display:"grid", gridTemplateColumns:"1fr 1fr 1fr auto", gap:12, alignItems:"center", borderLeft:"3px solid #d97706" }}>
                        <div>
                            ...
                            <div style={{ fontSize:10, color:"#9ca3af" }}>🕒 {timeAgo(d.createdAt)}</div>
                        </div>
                        <Chip color="#d97706">{DLABEL[d.donationType]||d.donationType}</Chip>
                        {d.amount ? <div style={{ fontSize:16, fontWeight:800, color:"#d97706" }}>💰 {fmtNum(d.amount)}đ</div> : <span style={{ color:"#9ca3af", fontSize:12 }}>Hiện vật</span>}
...
function AlertsPage({ sos, alerts }) {
    ...
    // Auto-generated alerts from data
        ...
        <div>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:16 }}>
                <div style={{ display:"flex", gap:10 }}>
                    <div style={{ background:"#fef2f2", border:"1px solid #fca5a5", borderRadius:10, padding:"8px 14px", fontSize:12, fontWeight:700, color:"#dc2626" }}>
                        🔴 {allAlerts.filter(a=>a.level==="critical").length} Nghiêm trọng
                    </div>
                    <div style={{ background:"#fffbeb", border:"1px solid #fcd34d", borderRadius:10, padding:"8px 14px", fontSize:12, fontWeight:700, color:"#d97706" }}>
                        🟡 {allAlerts.filter(a=>a.level==="warning").length} Cảnh báo
                    </div>
                </div>
                <button onClick={()=>setModal(true)} style={{ background:"#0f172a", color:"white", border:"none", borderRadius:10, padding:"9px 18px", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
                        ...
                        {autoAlerts.map(a=>{
                            ...
                            return (
                                <div key={a.id} style={{ background:cfg.bg, border:`1px solid ${cfg.color}30`, borderRadius:12, padding:"14px 16px", display:"flex", alignItems:"center", gap:12, borderLeft:`4px solid ${cfg.color}` }}>
                                    <span style={{ fontSize:20, animation:a.level==="critical"?"alertPulse 1s infinite":"none" }}>{cfg.icon}</span>
                                    <div style={{ flex:1 }}>
            ...
            <div>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                    {alerts.length===0 && <div style={{ background:"#f8fafc", border:"1px dashed #e5e7eb", borderRadius:12, padding:"30px", textAlign:"center", color:"#9ca3af", fontSize:13 }}>Chưa có cảnh báo thủ công nào. Bấm "+ Tạo cảnh báo" để thêm.</div>}
                    {alerts.map(a=>{
                        ...
                        return (
                            <div key={a.id} style={{ background:"#fff", border:"1px solid #f1f5f9", borderRadius:12, padding:"14px 16px", display:"flex", alignItems:"center", gap:12, borderLeft:`4px solid ${cfg.color}` }}>
                                ...
                                <div style={{ flex:1 }}>
                                    ...
                                    <div style={{ fontSize:12, color:"#6b7280", marginTop:2 }}>{a.message}</div>
                                    {a.province && <div style={{ fontSize:11, color:"#9ca3af", marginTop:2 }}>📍 {a.province}</div>}
                                </div>
                                ...
                                <button onClick={()=>deleteDocument("alerts",a.id)} style={{ background:"none", border:"none", cursor:"pointer", color:"#dc2626", fontSize:14 }}>🗑</button>
                            </div>
                        );
                    })}
                </div>
            ...
            <Modal open={modal} title="🔔 Tạo cảnh báo thủ công" onClose={()=>setModal(false)}>
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                    ...
                    <button onClick={handleSave} disabled={saving||!form.title||!form.message} style={{ background:"#0f172a", color:"white", border:"none", borderRadius:12, padding:13, fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit", opacity:saving?0.7:1, marginTop:8 }}>
    ...
    return (
        <div style={{ maxWidth:480 }}>
            <div style={{ background:"#fff", borderRadius:16, padding:24, border:"1px solid #f1f5f9", marginBottom:14 }}>
                <h3 style={{ fontSize:14, fontWeight:700, color:"#111827", margin:"0 0 16px" }}>⚙️ Cài đặt hệ thống</h3>
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                    ...
                    <button onClick={()=>{setSaved(true);setNewPw("");}} style={{ background:"#16a34a", color:"white", border:"none", borderRadius:10, padding:11, fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
                        {saved?"✅ Đã lưu (cập nhật trong code)":"💾 Lưu mật khẩu"}
                    </button>
            ...
            <div style={{ background:"#fff", borderRadius:16, padding:24, border:"1px solid #f1f5f9" }}>
                <h3 style={{ fontSize:14, fontWeight:700, color:"#111827", margin:"0 0 12px" }}>🔗 Về hệ thống</h3>
            ...
            <button onClick={onLogout} style={{ marginTop:14, width:"100%", background:"#fef2f2", color:"#dc2626", border:"1px solid #fca5a5", borderRadius:12, padding:12, fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
                🚪 Đăng xuất
            </button>
        </div>
    );
...
export default function AdminPage() {
    ...
    const alertCount  = alerts.filter(a=>a.active!==false).length + (sos.filter(r=>r.status==="urgent").length>=5?1:0);
    ...
    const PAGE_COMPONENTS = {
        ...
        stations:   <div style={{ textAlign:"center", padding:"80px 0", color:"#9ca3af" }}>🏠 Tính năng Điểm cứu trợ sẽ được thêm sớm</div>,
    ...
    const PAGE_TITLES = {
        dashboard:"Dashboard",sos:"Điều phối SOS",map:"Bản đồ Live",volunteers:"Tình nguyện viên",
        supplies:"Vật tư cứu trợ",stations:"Điểm cứu trợ",donations:"Quyên góp",
        stats:"Thống kê",alerts:"Cảnh báo",settings:"Cài đặt"
    };
            ...
            <aside style={{ width:220, background:"#0f172a", display:"flex", flexDirection:"column", flexShrink:0 }}>
                ...
                <div style={{ padding:"20px 16px 16px", borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                        <div style={{ width:36, height:36, borderRadius:10, background:"linear-gradient(135deg,#16a34a,#15803d)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18 }}>🛡️</div>
                        <div>
                ...
                <div style={{ padding:"12px 16px", borderTop:"1px solid rgba(255,255,255,0.06)" }}>
                    ...
                    <button onClick={handleLogout} style={{ width:"100%", background:"rgba(220,38,38,0.12)", color:"#f87171", border:"1px solid rgba(220,38,38,0.2)", borderRadius:8, padding:"8px", fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>
                        Đăng xuất
                    </button>
                </div>
            </aside>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                        {urgentCount>0 && (
                            <div style={{ background:"#fef2f2", border:"1px solid #fca5a5", borderRadius:10, padding:"6px 12px", fontSize:12, fontWeight:700, color:"#dc2626", display:"flex", alignItems:"center", gap:6 }}>
                                <span style={{ animation:"alertPulse 1s infinite" }}>🚨</span>
                                {urgentCount} khẩn cấp
                            </div>
                        )}
                        <a href="/" style={{ fontSize:12, color:"#6b7280", textDecoration:"none", padding:"6px 12px", border:"1px solid #e5e7eb", borderRadius:8 }}>← Trang chủ</a>
                    </div>
                </div>
