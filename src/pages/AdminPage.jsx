...
import {
    Shield, LayoutDashboard, AlertTriangle, AlertOctagon,
    Map, MapPin, Users, User, Package, Home, Gift,
    BarChart2, Bell, Settings, CheckCircle, CheckCircle2,
    ClipboardList, Search, Clock, Trash2, Phone,
    DollarSign, LogOut, Loader2, Plus, ArrowRight,
    ChevronRight, X, Save, Megaphone, Bot, Wrench,
    Droplets, Utensils, Shirt, Pill, Truck, Activity,
    Ambulance, Siren, RefreshCw, Flag, Info, Edit3,
    UserCheck, Building2, Heart, Circle as LucideCircle,
    TriangleAlert, BadgeAlert, Zap, ShieldAlert,
} from "lucide-react";
...
const PROVINCES = ["Tất cả","TP. Đà Nẵng","TP. Huế","Quảng Ngãi","Quảng Nam","Quảng Trị","Khánh Hòa","Nghệ An","Hà Tĩnh","Bình Định","Gia Lai"];
const ALERT_LEVELS = {
...
function Chip({ children, color="#16a34a" }) {
    return (
        <span style={{ fontSize:10, padding:"2px 8px", borderRadius:12, background:`${color}18`, color, border:`1px solid ${color}40`, fontWeight:600 }}>
            {children}
        </span>
    );
}
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                    ...
                    {err && (
                        <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:"#f87171" }}>
                            <AlertTriangle size={13} /> Mật khẩu không đúng
                        </div>
                    )}
    ...
    return (
        <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14 }}>
                ...
                <StatCard Icon={CheckCircle2}    value={done}        label="Hoàn thành"     color="#16a34a" sub="đã xử lý" />
            ...
            <div style={{ background:"linear-gradient(135deg,#0f172a,#1e293b)", borderRadius:16, padding:20, display:"flex", gap:12, alignItems:"center" }}>
                ...
                {[
                    ...
                    {page:"supplies",label:"Vật tư",        Icon:Package},
                ].map(b=>(
            ...
            <div style={{ display:"flex", gap:8, marginBottom:16, flexWrap:"wrap", alignItems:"center" }}>
                ...
                <div style={{ marginLeft:"auto", position:"relative" }}>
                    ...
                    <input placeholder="Tìm kiếm..." value={search} onChange={e=>setSearch(e.target.value)}
                           style={{ border:"1px solid #e5e7eb", borderRadius:10, padding:"7px 12px 7px 30px", fontSize:12, outline:"none", fontFamily:"inherit", width:200, color:"#111827" }} />
                </div>
            </div>
                        ...
                        <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
                            <button onClick={()=>handleStatus(item)} disabled={busy===item.id} style={{ background:"#16a34a", color:"white", border:"none", borderRadius:8, padding:"6px 10px", fontSize:11, fontWeight:700, cursor:"pointer", fontFamily:"inherit", opacity:busy===item.id?0.6:1, whiteSpace:"nowrap", display:"flex", alignItems:"center", gap:4 }}>
                                {busy===item.id
                                    ...
                                    : <><ArrowRight size={11} /> {S[NEXT[item.status]]?.label||"Hỗ trợ"}</>
                                }
                            </button>
                ...
                {selected && (
                    <div style={{ background:"#f8fafc", border:"1px solid #e5e7eb", borderRadius:12, padding:12 }}>
                        ...
                        <div style={{ fontSize:11, color:"#6b7280", lineHeight:1.8 }}>
                            <div style={{ display:"flex", alignItems:"center", gap:5 }}><User size={11} /> {selected.name}</div>
                            <div style={{ display:"flex", alignItems:"center", gap:5 }}><Phone size={11} /> {selected.phone}</div>
                            <div style={{ display:"flex", alignItems:"center", gap:5 }}><MapPin size={11} /> {selected.address}</div>
                            <div style={{ display:"flex", alignItems:"center", gap:5 }}><Heart size={11} /> {selected.supportType}</div>
                            {selected.note && <div style={{ display:"flex", alignItems:"center", gap:5 }}><Edit3 size={11} /> {selected.note}</div>}
                        </div>
                        ...
                        <Marker key={item.id} position={[item.lat,item.lng]} icon={makeIcon(item.status)}
                            ...
                            <Popup>
                                <div style={{ minWidth:180 }}>
                                    ...
                                    <div style={{ fontSize:11, color:"#6b7280", display:"flex", alignItems:"center", gap:4 }}><MapPin size={10} /> {item.address}</div>
                                    <div style={{ fontSize:11, color:"#6b7280", display:"flex", alignItems:"center", gap:4, marginTop:2 }}><Heart size={10} /> {item.supportType}</div>
                ...
                {filtered.map(v=>(
                    <div key={v.id} style={{ background:"#fff", border:"1px solid #f1f5f9", borderRadius:14, padding:16, borderLeft:"3px solid #16a34a" }}>
                        ...
                        <div style={{ display:"flex", gap:6, marginBottom:8 }}>
                            ...
                            <Chip color="#2563eb"><span style={{ display:"flex", alignItems:"center", gap:3 }}><Clock size={9} /> {v.availability==="fulltime"?"Full":(v.availability==="weekend"?"Cuối tuần":(v.availability==="evening"?"Buổi tối":"Linh hoạt"))}</span></Chip>
                        </div>
...
function SuppliesPage({ supplies }) {
    ...
    const [form, setForm]     = useState({ name:"", category:"food", quantity:"", unit:"kg", minQuantity:"", location:"TP. Đà Nẵng", note:"" });
    ...
    const handleSave = async () => {
        ...
        setSaving(false);
        setModal(false);
        setForm({ name:"", category:"food", quantity:"", unit:"kg", minQuantity:"", location:"TP. Đà Nẵng", note:"" });
    };
            ...
            <Modal open={modal} title="Thêm vật tư cứu trợ" onClose={()=>setModal(false)}>
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                    ...
                    <button onClick={handleSave} disabled={saving||!form.name||!form.quantity}
                            style={{ background:"#16a34a", color:"white", border:"none", borderRadius:12, padding:13, fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit", opacity:saving?0.7:1, marginTop:8, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                ...
                {donations.map(d=>(
                    <div key={d.id} style={{ background:"#fff", border:"1px solid #f1f5f9", borderRadius:14, padding:"14px 18px", display:"grid", gridTemplateColumns:"1fr 1fr 1fr auto", gap:12, alignItems:"center", borderLeft:"3px solid #d97706" }}>
                        ...
                        {d.amount
                            ? <div style={{ fontSize:16, fontWeight:800, color:"#d97706", display:"flex", alignItems:"center", gap:5 }}><DollarSign size={14} /> {fmtNum(d.amount)}đ</div>
...
function StatsPage({ sos, volunteers, donations }) {
    ...
    const byType   = useMemo(()=>{ const m={}; sos.forEach(r=>{const t=r.supportType||"Khác";m[t]=(m[t]||0)+1;}); return Object.entries(m).sort((a,b)=>b[1]-a[1]).slice(0,6); }, [sos]);
    const byProv   = useMemo(()=>{ const m={}; sos.forEach(r=>{const p=r.province||"Khác";m[p]=(m[p]||0)+1;}); return Object.entries(m).sort((a,b)=>b[1]-a[1]).slice(0,8); }, [sos]);
...
function AlertsPage({ sos, alerts }) {
    const [modal, setModal]   = useState(false);
    const [form, setForm]     = useState({ title:"", message:"", level:"warning", province:"TP. Đà Nẵng" });
    const [saving, setSaving] = useState(false);
    const set = (k,v) => setForm(f=>({...f,[k]:v}));
    ...
    const handleSave = async () => {
        ...
        setForm({ title:"", message:"", level:"warning", province:"TP. Đà Nẵng" });
    };
                        ...
                        return (
                            <div key={a.id} style={{ background:"#fff", border:"1px solid #f1f5f9", borderRadius:12, padding:"14px 16px", display:"flex", alignItems:"center", gap:12, borderLeft:`4px solid ${cfg.color}` }}>
                                ...
                                <div style={{ flex:1 }}>
                                    ...
                                    <div style={{ fontSize:12, color:"#6b7280", marginTop:2 }}>{a.message}</div>
                                    {a.province && <div style={{ fontSize:11, color:"#9ca3af", marginTop:2, display:"flex", alignItems:"center", gap:4 }}><MapPin size={10} /> {a.province}</div>}
                                </div>
            ...
            <Modal open={modal} title="Tạo cảnh báo thủ công" onClose={()=>setModal(false)}>
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                    ...
                    <button onClick={handleSave} disabled={saving||!form.title||!form.message}
                            style={{ background:"#0f172a", color:"white", border:"none", borderRadius:12, padding:13, fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit", opacity:saving?0.7:1, marginTop:8, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
                        {saving ? <><Loader2 size={16} style={{ animation:"spin 1s linear infinite" }} /> Đang lưu...</>
    ...
    return (
        <div style={{ maxWidth:480 }}>
            <div style={{ background:"#fff", borderRadius:16, padding:24, border:"1px solid #f1f5f9", marginBottom:14 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                    ...
                    <button onClick={()=>{setSaved(true);setNewPw("");}}
                            style={{ background:"#16a34a", color:"white", border:"none", borderRadius:10, padding:11, fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
                        {saved ? <><CheckCircle2 size={15} /> Đã lưu (cập nhật trong code)</> : <><Save size={15} /> Lưu mật khẩu</>}
                    </button>
            ...
            <button onClick={onLogout}
                    style={{ marginTop:14, width:"100%", background:"#fef2f2", color:"#dc2626", border:"1px solid #fca5a5", borderRadius:12, padding:12, fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
...
export default function AdminPage() {
    ...
    const PAGE_COMPONENTS = {
        ...
        stations:   <div style={{ textAlign:"center", padding:"80px 0", color:"#9ca3af" }}><Home size={48} style={{ margin:"0 auto 16px", display:"block", opacity:0.3 }} /><p>Tính năng Điểm cứu trợ sẽ được thêm sớm</p></div>,
            ...
            <aside style={{ width:220, background:"#0f172a", display:"flex", flexDirection:"column", flexShrink:0 }}>
                    ...
                    {SIDEBAR_MENU.map(m=>{
                        ...
                        return (
                            <button key={m.key} onClick={()=>setPage(m.key)} style={{
                                ...
                                {badge>0 && (
                                    <span style={{ background:"#dc2626", color:"white", borderRadius:99, padding:"1px 7px", fontSize:10, fontWeight:700, minWidth:20, textAlign:"center" }}>
                                        {badge}
                                    </span>
                                )}
                            </button>
                ...
                <div style={{ padding:"12px 16px", borderTop:"1px solid rgba(255,255,255,0.06)" }}>
                    ...
                    <button onClick={handleLogout}
                            style={{ width:"100%", background:"rgba(220,38,38,0.12)", color:"#f87171", border:"1px solid rgba(220,38,38,0.2)", borderRadius:8, padding:"8px", fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                        {urgentCount>0 && (
                            <div style={{ background:"#fef2f2", border:"1px solid #fca5a5", borderRadius:10, padding:"6px 12px", fontSize:12, fontWeight:700, color:"#dc2626", display:"flex", alignItems:"center", gap:6 }}>
                                <Siren size={14} style={{ animation:"alertPulse 1s infinite" }} />
                                {urgentCount} khẩn cấp
                            </div>
                        )}
                        <a href="/" style={{ fontSize:12, color:"#6b7280", textDecoration:"none", padding:"6px 12px", border:"1px solid #e5e7eb", borderRadius:8, display:"flex", alignItems:"center", gap:5 }}>
